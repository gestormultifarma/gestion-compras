...
def procesar_pdv(codigo_pdv, nombre_pdv, pdv_sk):
    ...
    try:
        ...
        # CORREGIDO: Consulta para buscar producto_sk con el nombre correcto de la columna (codigo en lugar de codigo_producto)
        query_productos = text(f"""
            UPDATE {nombre_tabla_temp} t
            JOIN dim_producto p ON t.codigo_producto = p.codigo
