# cargar_ventas_unidades.py
from sqlalchemy import create_engine, text
from utils.db_connection import get_mysql_url

def cargar_ventas_unidades():
    """
    Carga los datos reales de ventas en unidades (venta_unidad) desde las tablas 
    de staging en la tabla fact_rotacion, utilizando estos valores como rotación.
    """
    ...
    try:
        with engine.connect() as connection:
            print("Actualizando fact_rotacion con datos reales de ventas en unidades...")
            ...
            DROP TABLE IF EXISTS temp_rotacion_datos
            """))
            
            connection.execute(text("""
            CREATE TEMPORARY TABLE temp_rotacion_datos (
                codigo_pdv VARCHAR(20),
                nombre_pdv VARCHAR(100),
                codigo_producto VARCHAR(50),
                nombre_producto VARCHAR(255),
                periodo INT,
                venta_unidad DECIMAL(10,2)
            )
            """))
            connection.commit()
            ...
            for i, (tabla,) in enumerate(tablas_rotacion):
                ...
                try:
                    ...
                    # Obtener datos de ventas en unidades
                    query = f"""
                    INSERT INTO temp_rotacion_datos (
                        codigo_pdv, 
                        nombre_pdv,
                        codigo_producto,
                        nombre_producto,
                    ...
                    SELECT 
                        ...
                        {periodo} as periodo,
                        COALESCE(venta_unidad, 0) as venta_unidad
                    FROM {tabla}
                    """
            ...
            SELECT 
                ...
                SUM(CASE WHEN t.periodo = 1 THEN t.venta_unidad ELSE 0 END) as rotacion_m1,
                SUM(CASE WHEN t.periodo = 2 THEN t.venta_unidad ELSE 0 END) as rotacion_m2,
                SUM(CASE WHEN t.periodo = 3 THEN t.venta_unidad ELSE 0 END) as rotacion_m3
            FROM temp_rotacion_datos t
            ...
            print("\nEjemplos de registros con ventas:")
            print("=" * 100)
