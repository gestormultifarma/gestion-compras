# analysis/extractor/extractor_fact_rotacion_updated.py
...
class ExtractorFactRotacion:
    ...
    y prepararlos para la tabla fact_rotacion.
    """
    ...
    def extraer(self):
        """
        Extrae datos de las tablas stg_rotacion_* existentes.
    ...
    def _obtener_datos_tabla(self, nombre_tabla, pdv_info):
        ...
        if not df.empty:
            ...
            df['pdv_sk'] = pdv_info.get('pdv_sk')
                ...
                if self.logger:
                    self.logger.info(f"✓ Procesados {len(df)} registros de tabla {nombre_tabla}")
